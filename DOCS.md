...
| `placeLoop(node, cx, topY)` | Размещает шестиугольник + тело. Возвращает `visBot` — максимальный Y всех элементов включая ветви внутренних условий |
...
| `backPath(fromX, fromY, loopLX, loopCY)` | Маршрут back-стрелки: вниз → влево до outerX → вверх → вправо в левую точку шестиугольника |

**Маршрутизация стрелок для `if/else` внутри цикла:**

При наличии `else` обе ветви (YES и NO) сходятся в JOIN-точку по центру, из которой идёт **одна** back-стрелка в левую точку шестиугольника. Exit-стрелка цикла использует `exitBendX` (установленный `layoutCond`) чтобы обойти правую ветвь.
...
- Поддержка `while` цикла
- Вложенные циклы (loop внутри loop)
- Вложенные условия (if внутри if)
- Поддержка функций (`def`)
- `elif` (сейчас парсится как вложенный `if` в `else`)
